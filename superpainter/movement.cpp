		...
		if (state == GLUT_DOWN) {
			if (mod & GLUT_ACTIVE_ALT) {
				...
				case sys: {
					...
					graphList.ptail->setColorInside(colorInside_buff[0], colorInside_buff[1], colorInside_buff[2], trans);
					graphList.ptail->setColorBorder(colorBorder_buff[0], colorBorder_buff[1], colorBorder_buff[2], trans);
					refreshStatus();
					break;
				}
				case drawing: {
			...
			if (now_op == drawing) {
				switch (choosedType) {
					...
					case 5: {
						...
						double c = 0.551915024494;
						vertice_node * temp = graphList.ptail->vhead->next;

						double xmin = min(temp->x, x);
						double xmax = max(temp->x, x);
						double ymin = min(temp->y, y);
						double ymax = max(temp->y, y);

						if (xmax - xmin >= 2 * SMOOTH && ymax - ymin >= 2 * SMOOTH) {
							smooth = SMOOTH;
						}
						else
						{
							smooth = min(xmax - xmin, ymax - ymin) / 2;
						}

						//1
						temp->x = xmin + smooth;
						temp->y = ymin;
						temp->left->x = temp->x - smooth * (1 - c);
						temp->left->y = ymin;
						temp->right->x = temp->x + smooth * (1 - c);
						temp->right->y = temp->y;
						//2
						graphList.ptail->addPoint(xmax - smooth , ymin);
						temp = temp->next;
						temp->left->x = temp->x - smooth * c;
						temp->left->y = temp->y;
						temp->right->x = temp->x + smooth * c;
						temp->right->y = temp->y;
						//3
						graphList.ptail->addPoint(xmax, ymin + smooth);
						temp = temp->next;
						temp->left->x = xmax;
						temp->left->y = temp->y - smooth * (1 - c);
						temp->right->x = temp->x;
						temp->right->y = temp->y + smooth * (1 - c);
						//4
						graphList.ptail->addPoint(xmax, ymax - smooth);
						temp = temp->next;
						temp->left->x = temp->x;
						temp->left->y = temp->y - smooth * c;
						temp->right->x = temp->x;
						temp->right->y = temp->y + smooth * c;
						//5
						graphList.ptail->addPoint(xmax - smooth, ymax);
						temp = temp->next;
						temp->left->x = temp->x + smooth * c;
						temp->left->y = temp->y;
						temp->right->x = temp->x - smooth * c;
						temp->right->y = temp->y;
						//6
						graphList.ptail->addPoint(xmin + smooth, ymax);
						temp = temp->next;
						temp->left->x = temp->x + smooth * c;
						temp->left->y = temp->y;
						temp->right->x = temp->x - smooth * c;
						temp->right->y = temp->y;
						//7
						graphList.ptail->addPoint(xmin, ymax - smooth);
						temp = temp->next;
						temp->left->x = temp->x;
						temp->left->y = temp->y + smooth * c;
						temp->right->x = temp->x;
						temp->right->y = temp->y - smooth * c;
						//8
						graphList.ptail->addPoint(xmin, ymin + smooth);
						temp = temp->next;
						temp->left->x = temp->x;
						temp->left->y = temp->y + smooth * c;
						temp->right->x = temp->x;
...
void my_PassiveMotion(double x, double y) {
	...
	if (target != NULL) {
		...
		if (now_op == drawing) {
			...
			if (temp != NULL && x < temp->xmax && x > temp->xmin && y < temp->ymax && y > temp->ymin) {
				coverageMenu = glutCreateMenu(coverageMenuEvent);
				glutAddMenuEntry("up", 1);
				glutAddMenuEntry("down", 2);
				glutAddMenuEntry("top", 3);
				glutAddMenuEntry("bottom", 4);

				lineWidthMenu = glutCreateMenu(lineWidthMenuEvent);
				glutAddMenuEntry("2", 5);
				glutAddMenuEntry("4", 6);
				glutAddMenuEntry("6", 7);
				glutAddMenuEntry("8", 8);

				menu_id = glutCreateMenu(myMenu);
				glutAddMenuEntry("copy", 0);
				glutAddSubMenu("coverage", coverageMenu);
...
int checkPressButton(double x, double y) {
	...
	if (target >= &leftTop[0] && target <= &leftTop[LEFTTOP - 1]) {
		...
		else if (strcmp(temp, "cirsquare") == 0) choosedType = 5;
	}
...
void showMoving() {
	if (now_op == drawing) {
		if (choosedType == 5) {
			int n = 36;
			double xmin = min(newx, graphList.ptail->vtail->x);
			double ymin = min(newy, graphList.ptail->vtail->y);

			if (fabs(newx - graphList.ptail->vtail->x) >= 2 * SMOOTH && fabs(newy - graphList.ptail->vtail->y) >= 2 * SMOOTH) {
				smooth = SMOOTH;
			}
			else
			{
				smooth = min(fabs(newx - graphList.ptail->vtail->x), fabs(newy - graphList.ptail->vtail->y)) / 2;
			}
			if (graphList.ptail->solid) {
				...
				{
					glVertex2d((smooth * cos(2 * 3.1415926*i / n) + xmin + smooth)*standard - (standard - 1) * ww / 2, (smooth * sin(2 * 3.1415926*i / n) + ymin + smooth)*standard - (standard - 1) * wh / 2);
				}
				xmin += fabs(newx - graphList.ptail->vtail->x) - smooth * 2;
				for (int i = n * 3 / 4; i<n; i++)
				{
					glVertex2d((smooth * cos(2 * 3.1415926*i / n) + xmin + smooth)*standard - (standard - 1) * ww / 2, (smooth * sin(2 * 3.1415926*i / n) + ymin + smooth)*standard - (standard - 1) * wh / 2);
				}
				ymin += fabs(newy - graphList.ptail->vtail->y) - smooth * 2;
				for (int i = 0; i<n / 4; i++)
				{
					glVertex2d((smooth * cos(2 * 3.1415926*i / n) + xmin + smooth)*standard - (standard - 1) * ww / 2, (smooth * sin(2 * 3.1415926*i / n) + ymin + smooth)*standard - (standard - 1) * wh / 2);
				}
